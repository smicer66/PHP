    ...
    <td colspan="2"><table width="100%" border="0" cellpadding="5" cellspacing="5" class="cellType3">
      <tr>
        ...
        <td valign="top"><table width="100%" border="0" cellspacing="5" cellpadding="5">
          <tr>
            <td align="left" valign="bottom" bgcolor="#F8F8F8" class="cellType6"><span class="headers">Component Manager - Menu Creator </span></td>
          </tr>
          <tr>
			...
			else
			{
			 ...
			 ?>" method="post" enctype="multipart/form-data">
                      <table width="100%" border="0" cellspacing="5" cellpadding="5">
                        <tr>
                          <td width="24%" class="cellType11">Menu Name: </td>
                          <td width="76%" class="cellType10"><?php
		if((isset($_REQUEST['mod'])) && ($_REQUEST['mod']==1) && (isset($_REQUEST['id'])) && (is_numeric($_REQUEST['id'])==1))
		{
			echo "<input type='text' name='menuName' value='".getRealData($resultMOD['name'])."'/>";
		}
		else
		{
			echo "<input type='text' name='menuName' />";
		}
		?></td>
                        </tr>
                        <tr>
                          <td class="cellType11">Menu Type: </td>
                          <td class="cellType10"><select name="menuType">
                              <?php
	  echo append_option_menu_type($resultMOD['type']);
	  
	  ?>
                            </select>                          </td>
                        </tr>
                        <tr>
                          <td class="cellType11">Parent Menu: </td>
                          <td class="cellType10"><select name="menuParent">
                              <?php
	  echo	append_option_menu_parent($resultMOD['parentId']);
	  ?>
                          </select></td>
                        </tr>
                        <tr>
        ...
        if((isset($_REQUEST['mod'])) && ($_REQUEST['mod']==1) && (isset($_REQUEST['id'])) && (is_numeric($_REQUEST['id'])==1))
		{
			echo "<input name='UpdateMenu' type='submit' id='Update' value='Update' />";
		}
		else
		{
			echo "<input name='CreateMenu' type='submit' id='Create' value='Create' />";
		}          
					?></td>
                        </tr>
                      </table>
