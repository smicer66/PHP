...
{
	$resultMOD = $menu->getMenuDetails($_REQUEST['id']);
}
?>
    ...
    <td colspan="2"><table width="100%" border="0" cellpadding="5" cellspacing="5" class="cellType3">
      <tr>
        ...
        <td valign="top"><table width="100%" border="0" cellspacing="5" cellpadding="5">
          <tr>
            <td align="left" valign="bottom" bgcolor="#F8F8F8" class="cellType6"><span class="headers">Features Manager - Billing Costs Updater </span></td>
          </tr>
          <tr>
			...
			else
			{
			 ...
			 ?>" method="post" enctype="multipart/form-data">
                      <table width="100%" border="0" cellspacing="5" cellpadding="5">
                        <tr>
                          <td width="24%" class="cellType11">Transaction Type: </td>
                          <td width="76%" class="cellType10"><select name="transacType" id="transacType">
                              <?php
	  echo append_billing_type($_COOKIE['transacType']);
	  
	  ?>
                            </select>                          </td>
                        </tr>
                        <tr>
                          <td class="cellType11">Transaction Costs (Naira) per day:  </td>
                          <td class="cellType10"><?php
		
			echo "<input type='text' name='costs' value='".$_COOKIE['costs']."'/>";
		
		?><br /><span class="warningText">(e.g. 1000, 500. No commas)</span> </td>
                        </tr>
                        
                        <tr>
                          <td class="cellType11">Start Billing On: </td>
                          <td class="cellType10"><?php
		echo "<input name='startdate' readonly='readonly' type='text' id='startdate' value'".$_COOKIE['startdate']."' />";
		
		?>
				  
				  &nbsp;<img onclick="JACS.show(document.getElementById('startdate'),event)" src="../images/calendar.gif"/>&nbsp;<span class="warningText">(date format - yyyy-mm-dd)</span></td>
                        </tr>
                        <tr>
                          <td class="cellType11">&nbsp;</td>
                          <td class="cellType10">
                            <?php
        echo "<input name='CreateBillingCosts' type='submit' id='Create' value='Set' />";
