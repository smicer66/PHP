...
class Notification extends Feature
{
	...
	function getUserProjectMessage($fiid, $userId)
	{
		...
		$str="SELECT * FROM `".DEFAULT_DB_TBL_PREFIX."_messages` WHERE `status` = '1' AND receipientunread = '1' AND `receipient_id` = '".$userId."' AND `project_id` = '".$fiid."'";
	...
	function getUnreadMessages($userId=NULL)
	{
		...
		$str="SELECT * FROM `".DEFAULT_DB_TBL_PREFIX."_messages` WHERE `status` = '1'".$extraStr." AND receipientunread = '1'";
		//echo $str;
		$sql= $mysql->query($str);
		return $sql;	
	}
	...
	function setReadMessage($id)
	{
		...
		{
			$update="UPDATE `".DEFAULT_DB_TBL_PREFIX."_messages` SET `sourceunread` = 0 WHERE `id` = '".$id."' AND `sourceunread` = '1'";
		}
		else if($_SESSION['viewmsgtype']==0)
		{
			$update="UPDATE `".DEFAULT_DB_TBL_PREFIX."_messages` SET `receipientunread` = 0 WHERE `id` = '".$id."' AND `receipientunread` = '1'";
	...
	{
		CONTROLLER_NOTIFICATION::controlFrontPageDisplay();
	}
	
	function displayFeature($id=NULL)
	{
		CONTROLLER_NOTIFICATION::controlFlowProcess($id);
	}
	...
	function sendMessage($projId=NULL)
	{
		...
		if(USER::getUserTypeId($_SESSION['uid'])==USERTYPE::getUserTypeId('Developer'))
		{
			...
			$entryString="INSERT INTO `".DEFAULT_DB_TBL_PREFIX."_messages` (`id` ,`unique_id`,`project_id` ,`subject` ,`source_id` ,`receipient_id`,`details` ,`creation_date` ,`status`,`receipientunread`, `sourceunread`)VALUES ('' , UUID(),'".$projId."', '".$subject."', '".$_SESSION['uid']."', '".$recp_results['id']."', '".$details."', '".$createDate."', 1, 1, 1)";
			$sql= $mysql->query($entryString);
							
			$siteDet=SITE::getDetails();
			$subject1='Message on '.$siteDet['churchName'].' from '.$senderDet['username'];
